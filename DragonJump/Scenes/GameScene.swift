...
class GameScene: SKScene {
    ...
    private func setupLabelScore() {
        ...
        scoreLabel.position = CGPoint(x: screenSize.width - 10,
                                      y: camera!.position.y + screenSize.height * 0.5 - 10)
    ...
    private func checkYPosition() {
        guard player.position.y < lastHeight
            else {
        ...
        lastHeight = player.position.y
        guard player.position.y < camera!.position.y - screenSize.height * 0.6 - player.size.height else { return }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .nanoseconds(1)) { [unowned self] in
            ...
            menuScene.screenShot = self.view?.pbTakeSnapshot()
